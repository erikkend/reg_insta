...
class Command(Filter):
    ...
    If filter is successful processed the :obj:`Command.CommandObj` will be passed to the handlers arguments.
    ...
    class CommandObj:
        ...
        Can be passed as keyword argument ``command`` to the handlers
        """
...
class CommandStart(Command):
    ...
    async def check(self, message: types.Message):
        """
        If deep-linking is passed to the filter result of the matching will be passed as ``deep_link`` to the handlers
...
class IsReplyFilter(BoundFilter):
    """
    Check if message is replied and send reply message to handlers
    """
    key = 'is_reply'
